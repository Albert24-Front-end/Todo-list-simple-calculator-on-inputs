...
function createToDo(objToDo) {
    ...
    todoItem.innerHTML = `
         <article class="post ${objToDo.ready ? "post__complete" : ""}">
            ...
            ${!objToDo.ready ? `<button class="post__ready" type="button" data-id="${objToDo.id}">✔</button>` : `<button class="post__delete" type="button" data-id="${objToDo.id}"><i class='bx bx-x'></i></button>`}
        </article>
    `;
...
function countToDo() {
    ...
    const uncompletedTasks = base.todo.filter(todo => !todo.ready).length;
    count.textContent = uncompletedTasks;
};
